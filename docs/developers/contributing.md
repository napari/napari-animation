...
Create a local environment and install the package in editable mode, along with the developer tools:
```sh
pip install -e . --group dev
```

We use [`pre-commit`](https://pre-commit.com) to run [`ruff`](https://github.com/astral-sh/ruff)
...
Linting and formatting behavior is configured in `pyproject.toml`.
